...
import { amountOf, base, betterPool, conditionsHold, holds, reading, run, walk, type World } from "./engine";
import { makeRule, type Action, type Rule } from "@/lib/model/autopilot";

const world: World = { health: 1.4, gap: 1.5, price: 48.79, wallet: 200, poolA: 300, poolB: 100, debt: 250 };
...
describe("reading and holds", () => {
  it("reads each subject from the world", () => {
    expect(reading("health_factor", world)).toBe(1.4);
  ...
  it("compares at or above, and at or below", () => {
    ...
    expect(holds({ kind: "health_factor", comparator: "lte", value: 1.3 }, world)).toBe(false);
...
describe("run", () => {
  it("supplies from the wallet into a pool", () => {
    ...
    expect(r.text).toBe("Supplied 50.00 USDC to Hub 2");
  });
